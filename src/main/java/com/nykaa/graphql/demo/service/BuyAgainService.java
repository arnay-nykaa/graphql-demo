...
public class BuyAgainService {
    ...
    private void getPDPInfo(Map<String, SkuInfo> skuList, Integer customerGroupId, String categoryTagFilter) throws JsonProcessingException {
        for (SkuInfo product : skuList.values()) {
            JsonNode productDetails = discoveryService.getProductDetails(product.getProductId(), customerGroupId, categoryTagFilter).path("response");
            product.setMinOrderQty(productDetails.get(NYKAA_API.MIN_ORDER_QTY).asInt());
            product.setMaxOrderQty(productDetails.get(NYKAA_API.MAX_ORDER_QTY).asInt());
        }
                    ...
                    for (JsonNode shipment : shipmentDetail) {
                        ...
                        if (itemList.isArray()) {
                            for (JsonNode item : itemList) {
                                addSkuDataToList(item, order, skuList);
                            }
                        }
                    }
                }
                JsonNode remainingDetail = order.path(OMS.REMAINING_DETAIL);
                if (remainingDetail.isArray()) {
                    for (JsonNode remainder : shipmentDetail) {
                        JsonNode itemList = remainder.path(OMS.ITEM_LIST);
                        if (itemList.isArray()) {
                            for (JsonNode item : itemList) {
                                addSkuDataToList(item, order, skuList);
                            }
    ...
    private void addSkuDataToList(JsonNode item, JsonNode order, Map<String, SkuInfo> skuList) {
